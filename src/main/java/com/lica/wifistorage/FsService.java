...
import com.lica.wifistorage.server.SessionThread;
import com.lica.wifistorage.server.TcpListener;
...
public class FsService extends Service implements Runnable {
    ...
    static public final String ACTION_STARTED = "com.lica.wifistorage.FTPSERVER_STARTED";
    static public final String ACTION_STOPPED = "com.lica.wifistorage.FTPSERVER_STOPPED";
    static public final String ACTION_FAILEDTOSTART = "com.lica.wifistorage.FTPSERVER_FAILEDTOSTART";

    // RequestStartStopReceiver listens for these actions to start/stop this server
	static public final String ACTION_START_FTPSERVER = "com.lica.wifistorage.ACTION_START_FTPSERVER";
    static public final String ACTION_STOP_FTPSERVER = "com.lica.wifistorage.ACTION_STOP_FTPSERVER";
    ...
    public void onDestroy() {
        ...
        Logger.d(TAG, "FsService.onDestroy() finished");
    }
    ...
    public void run() {
        ...
        try {
            ...
            Logger.w(TAG, "run: Unable to open port, bailing out.");
            stopSelf();
            sendBroadcast(new Intent(ACTION_FAILEDTOSTART));
            return;
        }
        ...
        while (!shouldExit) {
            if (wifiListener != null) {
                if (!wifiListener.isAlive()) {
                    Logger.d(TAG, "Joining crashed wifiListener thread");
                    try {
                        wifiListener.join();
                    } catch (InterruptedException e) {
                    }
    ...
    private void terminateAllSessions() {
        Logger.i(TAG, "Terminating " + sessionThreads.size() + " session thread(s)");
        synchronized (this) {
    ...
    public static boolean isConnectedToLocalNetwork() {
        ...
        if (connected == false) {
            Logger.d(TAG, "Device not connected to a network, see if it is an AP");
            WifiManager wm = (WifiManager) context.getSystemService(Context.WIFI_SERVICE);
            try {
    ...
    /**
     * The FsService must know about all running session threads so they can be
    ...
    public void registerSessionThread(SessionThread newSession) {
        ...
        synchronized (this) {
            ...
            for (SessionThread sessionThread : sessionThreads) {
                if (!sessionThread.isAlive()) {
                    Logger.d(TAG, "Cleaning up finished session...");
                    try {
                        sessionThread.join();
                        Logger.d(TAG, "Thread joined");
                        toBeRemoved.add(sessionThread);
                        sessionThread.closeSocket(); // make sure socket closed
                    } catch (InterruptedException e) {
                        Logger.d(TAG, "Interrupted while joining");
        ...
        Logger.d(TAG, "Registered session thread");
    }
