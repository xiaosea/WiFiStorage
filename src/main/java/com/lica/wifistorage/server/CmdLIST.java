...
public class CmdLIST extends CmdAbstractListing implements Runnable {
    ...
    public void run() {
        ...
        mainblock: {
            ...
            Logger.d(TAG, "LIST parameter: " + param);
            while (param.startsWith("-")) {
                // Skip all dashed -args, if present
                Logger.d(TAG, "LIST is skipping dashed arg " + param);
        ...
        if (errString != null) {
            ...
            Logger.d(TAG, "LIST failed with: " + errString);
        } else {
            Logger.d(TAG, "LIST completed OK");
        }
    ...
    protected String makeLsString(File file) {
        ...
        if (!file.exists()) {
            Logger.i(TAG, "makeLsString had nonexistent file");
            return null;
        }
        ...
        if (lastNamePart.contains("*") || lastNamePart.contains("/")) {
            Logger.i(TAG, "Filename omitted due to disallowed character");
            return null;
        } else {
            // The following line generates many calls in large directories
            // staticLogger.l(Logger.DEBUG, "Filename: " + lastNamePart);
        }
