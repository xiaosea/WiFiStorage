...
import com.lica.wifistorage.Defaults;
...
public class SessionThread extends Thread {
    ...
    // myLogger.l(Logger.DEBUG, "Setting PORT dest to " +
    ...
    public boolean sendViaDataSocket(String string) {
        try {
            ...
            Logger.d(TAG, "Using data connection encoding: " + encoding);
            return sendViaDataSocket(bytes, bytes.length);
        } catch (UnsupportedEncodingException e) {
            Logger.e(TAG, "Unsupported encoding for data socket send");
            return false;
        }
    }
    ...
    public boolean sendViaDataSocket(byte[] bytes, int start, int len) {
        ...
        if (dataOutputStream == null) {
            Logger.i(TAG, "Can't send via null dataOutputStream");
            return false;
        }
        if (len == 0) {
        ...
        try {
            ...
            Logger.i(TAG, "Couldn't write output stream for data socket");
    ...
    public int receiveFromDataSocket(byte[] buf) {
        ...
        if (dataSocket == null) {
            Logger.i(TAG, "Can't receive from null dataSocket");
            return -2;
        }
        if (!dataSocket.isConnected()) {
            Logger.i(TAG, "Can't receive from unconnected socket");
            return -2;
        }
        InputStream in;
        try {
            ...
            Logger.i(TAG, "Error reading data socket");
            return 0;
        }
    ...
    public boolean startUsingDataSocket() {
        try {
            ...
            if (dataSocket == null) {
                Logger.i(TAG, "dataSocketFactory.onTransfer() returned null");
                return false;
            }
            dataOutputStream = dataSocket.getOutputStream();
            return true;
        } catch (IOException e) {
            Logger.i(TAG, "IOException getting OutputStream for data socket");
            dataSocket = null;
            return false;
        }
    }
    ...
    public void quit() {
        Logger.d(TAG, "SessionThread told to quit");
        closeSocket();
    }

    public void closeDataSocket() {
        Logger.d(TAG, "Closing data socket");
        if (dataOutputStream != null) {
            try {
    ...
    @Override
    public void run() {
        Logger.i(TAG, "SessionThread started");
        ...
        try {
            ...
            while (true) {
                ...
                if (line != null) {
                    ...
                    Logger.d(TAG, "Received line from client: " + line);
                    FtpCmd.dispatchCommand(this, line);
                } else {
                    Logger.i(TAG, "readLine gave null, quitting");
                    break;
                }
            }
        } catch (IOException e) {
            Logger.i(TAG, "Connection was dropped");
        }
        closeSocket();
    }
    ...
    public void writeBytes(byte[] bytes) {
        try {
            ...
            Logger.i(TAG, "Exception writing socket");
            closeSocket();
            return;
        }
    }
    ...
    public void writeString(String str) {
        ...
        try {
            ...
            Logger.e(TAG, "Unsupported encoding: " + encoding);
    ...
    public void authAttempt(boolean authenticated) {
        if (authenticated) {
            Logger.i(TAG, "Authentication complete");
            ...
            if (source == Source.PROXY) {
                ...
                authFails++;
                Logger.i(TAG, "Auth failed: " + authFails + "/" + MAX_AUTH_FAILS);
            }
            if (authFails > MAX_AUTH_FAILS) {
                Logger.i(TAG, "Too many auth fails, quitting session");
                quit();
            }
        }
    ...
    public void setWorkingDir(File workingDir) {
        try {
            ...
            Logger.i(TAG, "SessionThread canonical error");
        }
    }

    /*
     * public FsService getService() { return service; }
     * 
     * public void setService(FsService service) { this.service = service; }
     */
