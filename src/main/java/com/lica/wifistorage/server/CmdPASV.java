...
public class CmdPASV extends FtpCmd implements Runnable {
    ...
    public void run() {
        ...
        Logger.d(TAG, "PASV running");
        int port;
        if ((port = sessionThread.onPasv()) == 0) {
            // There was a problem opening a port
            Logger.e(TAG, "Couldn't open a port for PASV");
            sessionThread.writeString(cantOpen);
            return;
        }
        InetAddress addr = sessionThread.getDataSocketPasvIp();
        if (addr == null) {
            Logger.e(TAG, "PASV IP string invalid");
            sessionThread.writeString(cantOpen);
            return;
        }
        Logger.d(TAG, "PASV sending IP: " + addr.getHostAddress());
        if (port < 1) {
            Logger.e(TAG, "PASV port number invalid");
            sessionThread.writeString(cantOpen);
            return;
        }
        ...
        Logger.d(TAG, "PASV completed, sent: " + responseString);
    }
}
