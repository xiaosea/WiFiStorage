...
public abstract class CmdAbstractListing extends FtpCmd {
    ...
    public String listDirectory(StringBuilder response, File dir) {
        ...
        Logger.d(TAG, "Listing directory: " + dir.toString());
        ...
        Logger.d(TAG, "Dir len " + entries.length);
        try {
            ...
            Logger.e(TAG, "Unable to sort the listing: " + e.getMessage());
    ...
    protected String sendListing(String listing) {
        if (sessionThread.startUsingDataSocket()) {
            Logger.d(TAG, "LIST/NLST done making socket");
        } else {
        ...
        Logger.d(TAG, "Sent code 150, sending listing string now");
        if (!sessionThread.sendViaDataSocket(listing)) {
            Logger.d(TAG, "sendViaDataSocket failure");
            sessionThread.closeDataSocket();
            return "426 Data socket or network error\r\n";
        }
        sessionThread.closeDataSocket();
        Logger.d(TAG, "Listing sendViaDataSocket success");
