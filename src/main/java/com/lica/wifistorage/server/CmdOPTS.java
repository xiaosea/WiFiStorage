    ...
    public void run() {
        ...
        mainBlock: {
            if (param == null) {
                ...
                Logger.w(TAG, "Couldn't understand empty OPTS command");
                break mainBlock;
            }
            String[] splits = param.split(" ");
            if (splits.length != 2) {
                errString = "550 Malformed OPTS command\r\n";
                Logger.w(TAG, "Couldn't parse OPTS command");
                break mainBlock;
            }
            ...
            if (optName.equals("UTF8")) {
                ...
                if (optVal.equals("ON")) {
                    Logger.d(TAG, "Got OPTS UTF8 ON");
                    sessionThread.setEncoding("UTF-8");
                } else {
                    Logger.i(TAG, "Ignoring OPTS UTF8 for something besides ON");
                }
                break mainBlock;
            } else {
                Logger.d(TAG, "Unrecognized OPTS option: " + optName);
