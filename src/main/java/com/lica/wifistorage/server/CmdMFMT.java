...
public class CmdMFMT extends FtpCmd implements Runnable {
    ...
    @Override
    public void run() {
        Logger.d(TAG, "run: MFMT executing, input: " + mInput);
        String[] params = getParameter(mInput).split(" ");

        if (params.length != 2) {
            sessionThread.writeString("500 wrong number of parameters\r\n");
            Logger.d(TAG, "run: MFMT failed, wrong number of parameters");
            return;
        }
        ...
        try {
            ...
            Logger.d(TAG, "run: MFMT failed, unable to parse parameter time-val");
            return;
        }
        ...
        if (file.exists() == false) {
            ...
            Logger.d(TAG, "run: MFMT failed, file does not exist");
            return;
        }
        ...
        if (success == false) {
            ...
            Logger.d(TAG, "run: MFMT failed, unable to modify last modification time");
            // more info at
        ...
        Logger.d(TAG, "run: MFMT completed succesful");
    }
