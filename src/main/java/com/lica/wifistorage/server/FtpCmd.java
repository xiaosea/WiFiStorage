...
public abstract class FtpCmd implements Runnable {
    ...
    protected static void dispatchCommand(SessionThread session, String inputString) {
        ...
        for (int i = 0; i < cmdClasses.length; i++) {
            ...
            if (cmdClasses[i].getName().equals(verb)) {
                ...
                try {
                    ...
                    Logger.e(TAG, "FtpCmd subclass lacks expected " + "constructor ");
                    return;
                }
                try {
                    ...
                    Logger.e(TAG, "Instance creation error on FtpCmd");
                    return;
                }
            }
        }
        if (cmdInstance == null) {
            ...
            Logger.d(TAG, "Ignoring unrecognized FTP verb: " + verb);
            session.writeString(unrecognizedCmdMsg);
            return;
    ...
    static public String getParameter(String input, boolean silent) {
        ...
        if (!silent) {
            Logger.d(TAG, "Parsed argument: " + retString);
        }
        return retString;
    }
    ...
    public boolean violatesChroot(File file) {
        try {
            ...
            if (!canonicalPath.startsWith(canonicalChroot)) {
                Logger.i(TAG, "Path violated folder restriction, denying");
                Logger.d(TAG, "path: " + canonicalPath);
                Logger.d(TAG, "chroot: " + chroot.toString());
                return true; // the path must begin with the chroot path
            }
            return false;
        } catch (Exception e) {
            Logger.i(TAG, "Path canonicalization problem: " + e.toString());
            Logger.i(TAG, "When checking file: " + file.getAbsolutePath());
